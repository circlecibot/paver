...
def sh2(cmd, *args, **kw):
  """
  a shorthand function for `sh`ing, hoping to make sh calls prettier to write.

  command string is auto formatted from the `*args` and `**kwargs`.

  `ignore_error` is shortened to just `error`
  """
  cwd = kw.get("cwd", ".")
  err = not kw.pop("error", False)
  capture = kw.pop("capture", False)
  if len(args) > 0:
    cmd = cmd.format(*args)
  if len(kw) > 0:
    cmd = cmd.format(**kw)
  return sh(cmd, ignore_error=err, capture=capture, cwd=cwd)
